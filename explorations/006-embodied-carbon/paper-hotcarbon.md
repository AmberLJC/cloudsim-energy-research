> **HotCarbon 2026 condensed version (6-page target). Source: paper.md v0.8**
...
The AI industry's prevalent ~2-year GPU refresh cycle is carbon-suboptimal for inference workloads, which constitute the majority of deployed AI compute. Simply extending GPU inference server lifetimes from 2 to 4 years saves **20–52% lifecycle carbon** versus the industry norm across six grid carbon intensity scenarios (50–800 gCO₂/kWh), conditional on GPU embodied carbon ≥ 1,500 kgCO₂/node. We demonstrate this via finite-horizon dynamic programming applied to a parameterized fleet simulation. DP-Optimal planning saves **10–60%** for CPU fleets under a five-year norm depending on grid intensity. Against a Fixed-3yr baseline (reflecting actual hyperscaler cycles), DP-Optimal saves 7–23%. We derive a deployable threshold heuristic — extend GPU inference refresh from 2 to 4 years — capturing 89% of DP savings with no monitoring overhead. Critically, we show for the first time that the classical steady-state T* formula is invalid for staggered fleet deployments and can underperform the industry norm in practice. Disaggregating training and inference hardware refresh cycles is the primary actionable lever for reducing embodied carbon in AI infrastructure.
...
Data center sustainability efforts have long focused on operational carbon — electricity from servers and cooling. As hyperscalers procure increasing shares of renewable energy, this framing misses a growing component: the **embodied carbon** of manufacturing server hardware. Gupta et al. [1] showed that for organizations with clean-energy procurement, embodied manufacturing carbon now constitutes 50–80% of total server lifecycle emissions. For AI GPU nodes — carrying an estimated 3,000–5,000 kgCO₂eq per server due to advanced TSMC packaging and HBM memory [4, 5] — this fraction is even larger. A data center powered entirely by renewables still emits substantial CO₂ through continuous hardware procurement cycles.

The AI hardware market intensifies this problem. NVIDIA's ~2-year architectural cadence (A100→H100→H200→B200, 2020–2025) — delivering ~2× compute-per-watt per generation for inference — creates competitive pressure for uniform 2-year GPU refresh cycles. Yet **inference** (60–80% of deployed AI GPU compute [2]) can tolerate 4-year-old hardware; only training requires frontier generations. The industry norm treats inference and training identically, despite fundamentally different performance requirements.

**Research question:** What is the carbon-optimal server refresh cycle, and how much lifecycle carbon does the industry norm waste? The answer depends on whether optimal cycle length is computed via the classical steady-state T* formula — which assumes all servers are age-0 simultaneously — or via finite-horizon dynamic programming that accounts for real staggered fleet age distributions. We show these two approaches give systematically different answers, with T* being invalid for real fleets.

**Contributions:**
1. First simulation-based quantification of lifecycle carbon under DP-optimal vs industry norm refresh policies across CPU and GPU fleet scenarios and six grid carbon intensity scenarios.
2. First demonstration that classical T* analysis overestimates optimal cycle length for staggered fleet deployments and can *underperform* the industry norm in practice.
